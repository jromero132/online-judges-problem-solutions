...
// Submission link: https://vjudge.net/solution/33031791
...
bool miller_rabin(const T n, int iterations = 30, R rng = mt19937(chrono::steady_clock::now().time_since_epoch().count())){
	if(n < 4)
		return n == 2 || n == 3;
	...
	for(T p = uniform_int_distribution<T>(2, n - 1)(rng) ; iterations-- ; p = uniform_int_distribution<T>(2, n - 1)(rng))
